...
parameters such as local accretion rate, the disk can be strongly %
...
and the efficiency of accretion $\epsilon=0.1$. Hereafter, for scaled quantities we adopt 
the notation $y/10^X \to y_X$. 
...
\end{equation}
In the following we reserve $\Mdt$ for global, and $\mdt$ for local 
accretion rates. 
...
\begin{equation}
r_{{\rm in}}\simeq5\times10^{-3}M_{7}^{1/3}\mdt[0.1]^{1/3}T_{1500}^{-4/3}{\rm pc}\simeq5\times10^{3}M_{7}^{-2/3}\mdt[0.1]^{1/3}T_{1500}^{-4/3}\,{\rm r_{g}}.\label{eq:Rin}
\end{equation}
...
in previous sections. The most difficult to quantify unknown are the details of 
angular momentum transport (See Discussion).
We adopt an $\alpha-$disk theory of SS73 to include pressure on dust of the internal radiation 
produced by viscous dissipation. Mid- and near-IR radiation inside the disk is in thermodynamical 
equilibrium. Equation of state then is that of a mixture of ideal
gas and radiation:
...
\eqref{eq:Rin} and \eqref{eq:Rout} one can see that the size of this region relatively weakly depends on
$\dot{M}$
...
To see that radiation and convective fluxes are numerically similar, we first make corresponding estimate.
Choosing the fiducial values for 
