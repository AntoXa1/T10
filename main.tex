...
\newcommand{\mdt}[1][]{ \dot{M}_{{\rm loc}_{,#1}} }
...
\title{Connecting the torus to the Broad Line Region: a physical model for the radiative, convective dusty disk in AGN.}

\begin{abstract}
AGN accretion disk harbors and shields dust from external
illumination: at the mid-plane of the disk around a $M_{{\rm
BH}}=10^{7}M_{\odot}$ black hole, dust survives to approximately
$\lesssim0.1$pc compared to 0.5pc outside of the disk.  % The later
can be relevant to observations of hot dusty "ring" % in AGN 1068 We
construct a physical model of % "Radiative Dusty Disk": a region
...
surface.  Our model thus incorporates a "failed wind" scenario of
\citep{CzernyHryniewicz11} and the "compact torus" scenario of
\citet{BaskinLaor2018MNRAS} as limiting cases. The main conclusion is
that the disk's own radiation pressure on dust 
% on dust % associated
with 
% the disk's own near- and mid-infrared radiation, significantly
changes its vertical structure.  % Depending on the governing
parameters such as local accrtion rate, the disk can be strongly %
convective, slim or geometrically thick.  The relevance of our model
in the context of the AGN unification scheme and the origin of the
Broad Line Region is discussed.
...
surface'', surves as a natural barrier between the inner, mostly
...
(MIR) interferometric observations of nearby AGNs on the other hand
...
standpoint the most perswasive argument towards participation of
radiation pressure in shaping the outskirts of AGN transends from the
...
a $10^{7}M_{\odot}$ BH. in contrudiction to observations of the copious amounts of dust. 
...
Numerical simulations of thin disks are prohibitvely expensive even in a 
...
starting from already geometrically thick torus. A physical model should explain the torus's ubiquitousness and longevity, while
in numerical simulations the torsu exposed to external illumination is evolving on a dynamical time-scale.
...
% with putting the inner boundary or the torus to 
...
Magnetic or/and radiation driving have been proposed as a mechanisms behind the formaiton or BLR and the torus.
The line-driven wind (LDW) i.e. the wind driven by the radiation pressure in UV lines \citep{ProgaKallman2004,Murr05} 
is a possibility as well, however its launching radius and correspondingly the reultant characteristic 
...
is clearly an alternative or augmentive mechanism which can be the driving engine of the BLR and the torus. 
The majority are are relying on self-similar solutions \citep{Emmering92,Everett05,Keating12,KoniglKartje94} and thus intrincially
...
massive, polar, MHD-driven outflow. However, typical MHD flows have an approximate equapartition between magnetic energy density and 
...
% the obrervatioal the ubiquitnous of type 2 ANG calls for a robust mechanism  
% shich would shape their apperance presence of the
...
radiation pressure on dust graines that would expell a failed wind from the outer accretion atmosphere. 
Such failed wind then would be responsible for the formation of the broad line region seen
in Seyfert I like galaxies. Developping this idea further, \citep{BaskinLaor2018MNRAS}
...
Our major goal in this paper is to  how radiation pressure on dust grains changes the vertical structure of the disk itself. 
In previous works no such such calculatations to the vertical disk structure were made. 
We will show that such changes are important, and lead to varyfiable
results. 

We will calculate the disk structure in a region where such effect can significantly effect the vertical disk structure.
We first will develop an analitical model of an accretion disk, based on a modified $alpha$ disk approach, that include self-consistent contribution from gas pressure 
and radiation pressure on dust, and then show, that the active region approximately located between the radius of dust sublimation at
the disk mid-plane and the radius at which dust sublimes at the disk surface: $\sim 0.01-0.1$pc.
In a broad range of parameters, dust opacity results in a disk developing strong convection so that the energy transport along the vertical direction has significant
...
\begin{itemize}
    ...
    radiation shapes the vertical disk structure. In this paper, we call this part of the disk "Radiative Dusty Disk" (RDD).
    ...
    the disk properties. In particular, we argue, that convective RDD is a powerful source of turbulence.
    \item
    ...
    The later allows to make an argument that the entire concept of the type-2 AGN can be 
    attributed to a supper-critical dusty accretion in the RDD region, calculated in this paper.  
...
such disk should have the \textit{local} mass-accretion rate, $\mdt > \mdtcr$
...
due to very high UV opacity the radiation is stopped immidiately near the surface of the disk
...
\begin{figure}
\includegraphics[scale=0.8]{TurbDiskSketch} 
\caption{\label{fig:DiskSketch} Local radiation pressure creates convection
and is strong enough to push dust into polar region above the disk. Not to scale.
(A): Locally supercritical accretion: $\mdt{}\gtrsim \Mdt{glob}  (L_{{\rm bol}})$, where
...
where 
\begin{equation}
\mdtcr=\frac{4\pi cr}{\kappa_{d}}\simeq(18.4\,\kappa_{10}^{-1}-3.6\,\kappa_{50}^{-1})\,r_{0.01}\,M_{\odot}{\rm yr}^{-1}\mbox{,}\label{eq:MdotCritDust}
\end{equation}
...
1.5\times10^{5}\mdt{}_{0.1}^{4/9}\kappa_{10}^{2/9}M_{7}{}^{-\frac{2}{3}}\alpha_{0.1}{}^{-\frac{2}{9}}T_{1500}^{-10/9}\,{\rm r_{g}}\mbox{,}\label{eq:Rout}
\end{equation}
...
In this section we describe the details of our model. We adopt an
